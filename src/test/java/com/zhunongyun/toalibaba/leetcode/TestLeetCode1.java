...
public class TestLeetCode1 {
    ...
    void nullData() {
        ...
        AssertionErrors.assertTrue("无元素测试用例异常", "00".equals(DataUtils.translateIntToString(result)));
    }

    @Test
    void one() {
        int[] result = leetCode1.twoSum(oneList, 9);
        AssertionErrors.assertTrue("一个元素测试用例异常", "00".equals(DataUtils.translateIntToString(result)));
    }

    @Test
    void two() {
        int[] result = leetCode1.twoSum(twoList, 6);
        AssertionErrors.assertTrue("两个元素测试用例异常", "01".equals(DataUtils.translateIntToString(result)));
    }
}
